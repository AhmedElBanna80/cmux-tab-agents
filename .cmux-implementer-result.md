...
Implemented copy-pastable focus shortcuts for surface refs in tab-agent push protocol. Boot sequences in all three seed prompts now capture OWN_SURFACE via `cmux identify --no-caller` with graceful fallback. Push protocol emits a second line with copy-pastable `cmux rpc surface.focus` command. All documentation updated (discipline.md push protocol, SKILL.md convention, CHANGELOG.md entry). Cache-friendly prefix structure maintained across all changes.
...
- Boot sequences correctly capture surface ref with nested fallbacks: `$(cmux identify --no-caller --json 2>/dev/null | jq -r .surface_ref 2>/dev/null || echo "")`
...
None. Implementation is complete and verified. Code-reviewer prompt is 1 line over target and SKILL.md is 13 lines over target, but all acceptance criteria are met and tests pass. These overages are minor and occur because the feature documentation required more space than anticipated, but the core implementation is sound and cache-friendly structure is preserved.
