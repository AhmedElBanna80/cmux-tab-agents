...
Your result file MUST stay within these limits:

- **`summary`**: ≤ 200 words (2–3 sentences on what you did)
- **`concerns`**: ≤ 10 bullets, each ≤ 25 words
- **Total file**: ≤ 200 lines (excluding YAML frontmatter)

**Verbose detail** (test output, stack traces, diffs, logs): write to a sibling file (e.g., `.cmux-implementer-verification.txt`) and reference it from the result. DO NOT inline.

**Self-check before DONE:** Before writing the file, run `wc -l .cmux-implementer-result.md` and confirm it is within the cap. Edit if needed, then report.
...
>>>>>>> 8d66ed0 (feat(ISSUE-19): enforce result file size caps in reporting contract and seed prompts)
